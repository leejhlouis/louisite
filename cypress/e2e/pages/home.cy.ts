...
describe('home page specs', () => {
  ...
  it('should scroll down to featured projects when "featured projects" button is clicked', () => {
    ...
    cy.get('#projects h2').should('contain', 'Featured projects')

    const sampleData = projects[0]

    const sampleCard = cy.get('#projects ul li').first()
    sampleCard
      .contains(sampleData.title)
      .should('have.attr', 'href', sampleData.links[0].url ?? sampleData.links[1].url)
    sampleCard.get('#projects h3').should('contain', sampleData.title)
    sampleCard.get('#projects p').should('contain', sampleData.description)

    sampleCard.get('#projects footer ul:first-of-type li').should(el => {
      sampleData.techStacks.forEach((data, index) => {
        expect(el.eq(index)).to.contain(data)
      })
    })
    sampleCard.get('#projects footer ul:nth-child(2) li').should(el => {
